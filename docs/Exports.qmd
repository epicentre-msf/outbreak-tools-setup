...
It is possible to predefine exports, such as a specific export for a given MoH. The setup of exports is done in two different sheets:

- In the *Exports* sheet you define the export: numbers of exports and their metadata (button label, exported file name, type of headers etc.)  
...
This is a *yes/no* dropdown menu which controls whether *variables* that are tagged as "*Personal identifier*" in the *Dictionary* sheet should be included in the export.^[Remember, in the *Dictionary* sheet, any variable with *anything* written in the export columns will be exported in the corresponding exports (even if it is written "no").]
