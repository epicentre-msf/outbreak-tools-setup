...
-   Summary function: free text field for entering the **function** that will display the desired indicator.^[As in the dictionary, the function must be written in English, regardless of the language in which the rest of the setup is written.] 
...
**Summary function**: this is a free field in which to enter the formula corresponding to the desired analysis. The formula must always be written without the "=" sign and in English, whatever the language in which the **setup file** is written (see [formula](#formulas)).
...
**Format**: this is a static drop-down menu with a pre-selection of possible display formats. (see also [Format](#variable-format))
...
Definition in the **setup file**: ![](imgs/im-082-example-spatial-analysis.png)
